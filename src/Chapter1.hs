...
mid :: Int -> Int -> Int -> Int
mid x y z
  | x < y && x < z = if y < z then y else z -- when x is the min, decide between the other two
  | y < x && y < z = if x < z then x else z -- when y is the min, ...
  | z < x && z < y = if x < y then x else y -- when z is the min, ...
...
firstDigit n
  ...
  where
    ...
    dropLastDigit digits = div digits 10

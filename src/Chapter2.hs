...
-}
subList :: Int -> Int -> [a] -> [a]
subList start numberToTake list = take numberToTake (drop start list)
...
-}
takeEven :: [a] -> [a]
takeEven x = takeEvenGo x 0
  where
    takeEvenGo :: [a] -> Int -> [a]
    takeEvenGo [] _ = []
    takeEvenGo (x : xs) n
      | even n = [x] ++ takeEvenGo xs (n + 1)
      | otherwise = takeEvenGo xs (n + 1)
...
smartReplicate l = concat $ map (\x -> replicate x x) l -- is there a way to flatten this in one-pass? or does that matter since it is lazy eval'd?
...
pairMul :: Num a => ([a] -> [a] -> [a])
pairMul = zipWith (*) -- does eta-reduction mean removing both variables in this sense? or just one?
...
rotate n l
  | n < 0 = []
  | otherwise = take (length l) $ drop n $ cycle l -- is there a way to do this without '$'? parentheses?
...
rewind l = rewindGo l []
  where
    rewindGo :: [a] -> [a] -> [a]
    rewindGo [] acc = acc
    rewindGo (x : xs) acc = rewindGo xs (x : acc)
